...
async function run() {
    try {
        ...
        const taskCollection = client.db(process.env.mongoDbUSER).collection("taskCollection")
        const userCollection = client.db(process.env.mongoDbUSER).collection("userCollection")
        const paymentCollecton = client.db(process.env.mongoDbUSER).collection("paymentCollecton")
        const submittedTaskCollecton = client.db(process.env.mongoDbUSER).collection("submittedTaskCollecton")
        const withdrawCollection = client.db(process.env.mongoDbUSER).collection("withdrawCollection")
        const notification = client.db(process.env.mongoDbUSER).collection("notification")

        const verifyFirebaseToken = createVerifyFirebaseTokenMiddleware(userCollection);
        ...
        app.post("/users", async (req, res) => {
            ...
            const existUser = await userCollection.find({ email: email }).toArray()
        ...
        app.delete('/users/:id', verifyFirebaseToken, authorizeRoles('admin'), async (req, res) => {
        ...
        // update user role
        app.patch('/update-role/:id', verifyFirebaseToken, authorizeRoles('admin'), async (req, res) => {
            const id = req.params.id
        ...
        // add task
        app.post('/add-task', verifyFirebaseToken, authorizeRoles('buyer'), async (req, res) => {
        ...
        // my task
        app.get("/my-tasks/:email", verifyFirebaseToken, authorizeRoles('buyer'), async (req, res) => {
            const email = req.params.email
            const result = await taskCollection.find({ buyerEmail: email }).toArray()
            res.send(result)
        })

        // Update Task
        app.put('/update-my-task/:id', verifyFirebaseToken, authorizeRoles('buyer'), async (req, res) => {
            const id = req.params.id
        ...
        // delete task 
        app.delete("/my-tasks/:id", verifyFirebaseToken, authorizeRoles('buyer'), async (req, res) => {
            const id = req.params.id
        ...
        // payment 
        app.post('/payment', verifyFirebaseToken, authorizeRoles('buyer'), async (req, res) => {
            const { coins, amount, currency, transactionID, email, name } = req.body
        ...
        // Get Payment History
        app.get('/payment-history/:email', verifyFirebaseToken, authorizeRoles('buyer'), async (req, res) => {
            const email = req.params.email
            const result = await paymentCollecton.find({ email: email }).sort({ paidAt: -1 }).toArray()
            res.send(result)
        })

        // SUBMISSION task
        app.post('/submitted-task', verifyFirebaseToken, authorizeRoles('worker'), async (req, res) => {
            const submittedTask = req.body.submittedTask
            const newTask = { ...submittedTask, current_Date: new Date().toISOString() }
        ...
        // my Submitted task
        app.get('/my-submitted-task/:email', verifyFirebaseToken, async (req, res) => {
            const email = req.params.email
            const result = await submittedTaskCollecton.find({ worker_email: email }).sort({ current_Date: -1 }).toArray()
            res.send(result);
        })

        // Update Sumbitted Task
        app.patch('/update-submitted-task/:id', verifyFirebaseToken, async (req, res) => {
            const id = req.params.id
            const newStatus = req.body.newStatus

            const submittedTask = await submittedTaskCollecton.findOne({ _id: new ObjectId(id) })
            const worker = await userCollection.findOne({ email: submittedTask.worker_email })
            const task = await taskCollection.findOne({ _id: new ObjectId(submittedTask.task_id) })


            if (newStatus === "approved") {
                const updatedCoin = parseInt(worker.coin) + parseInt(submittedTask.payable_amount)
        ...
        // withdraw 
        app.post('/withdraw-request', verifyFirebaseToken, authorizeRoles('worker'), async (req, res) => {
        ...
        app.patch('/approveWithdraw/:withdrawID', verifyFirebaseToken, async (req, res) => {
            const withdrawID = req.params.withdrawID
            const withdrawReq = await withdrawCollection.findOne({ _id: new ObjectId(withdrawID) })
            ...
            if (updateStatus.modifiedCount === 1) {
                const user = await userCollection.findOne({ email: withdrawReq.worker_email })
        ...
        app.get('/notification/:email', verifyFirebaseToken, async (req, res) => {
            const email = req.params.email
            const result = await notification.find({ ToEmail: email }).sort({ Time: -1 }).toArray()
            res.send(result)
        })
